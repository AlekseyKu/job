...
// ISR: страница будет регенерироваться не чаще, чем раз в 60 секунд
...
// Функция для генерации статических маршрутов
export async function generateStaticParams() {
  // Запрашиваем только поле siteDomain для всех сайтов (минимальный набор данных)
  const res = await fetch(
    "https://cmsbase24.top/api/all-sites?fields[0]=siteDomain",
    { cache: "force-cache" }
  );
  ...
  const uniqueSites = Array.from(
    new Set(allSites.map((site: any) => normalizeDomain(site.siteDomain)))
  );
  // Возвращаем массив параметров для каждого динамического сегмента
  return uniqueSites.map((site) => ({ site }));
}

export default async function Page({ params }: { params: { site: string } }) {
  const { site } = params;
  // Получаем данные для конкретного сайта (SSG-данные, закэшированные на этапе сборки)
  ...
  const pageSections: any[] = siteData.page_sections || [];
